      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setForm({
            ...
            startAt: s.start_at ? new Date(s.start_at).toISOString().slice(0, 16) : '',
            endAt: s.end_at ? new Date(s.end_at).toISOString().slice(0, 16) : '',
            status: s.status || 'scheduled',
            notes: s.notes || ''
          });
          setScheduleTypeOption(isCustomType ? '__custom' : existingType);
          setCustomScheduleType(isCustomType ? existingType : '');
        }
      } catch (error) {
        // keep UI stable on failure
      }
    ...
    <AdminLayout>
      <div className="space-y-6 max-w-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-foreground">Edit Pricing Schedule</h1>
            <p className="text-muted-foreground text-sm mt-1">Update a time-based price</p>
          </div>
        </div>
